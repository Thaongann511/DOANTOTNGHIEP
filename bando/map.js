...
map = L.map('map', {
    ...
    crs: L.CRS.EPSG4326,
    center: [11.941176715906405, 108.4582902576784],
    zoom: 13,
    zoomControl: false
...
$(function() {
    $("#attributes").change(function() {
        ...
        if (value_type == 'xsd:short' || value_type == 'xsd:int' || value_type == 'xsd:double' || value_type == 'xsd:long') {
            ...
            operator1.options[3] = new Option('Tương đương với', ' = ');
...
function query() {
    ...
    $.getJSON(url, function(data) {
        ...
        caption.innerHTML = value_layer + " (Số thuộc tính: " + data.features.length + " )";
            ...
            success: function(xml) {
                ...
                $(xml).find('Layer').find('Layer').each(function() {
                    ...
                    if (name == layer_name) {
                        ...
                        if (bounds != undefined) { alert(layer_name + " đã được thêm vào bản đồ"); }
                    }
